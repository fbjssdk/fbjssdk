    ...
    var chrome = UserAgent.nativeApp()
      ...
      : ('<table>' +
        ...
        '          <input type="submit" value="' +
                     Intl.tx._("Kanselleer") + '"' +
        '            id="fb_dialog_loader_close"/>' +
        '        </label>' +
        '      </td>' +
        '      <td class="header_center">' +
        '        <div>' + Intl.tx._("Laai nog...") + '</div>' +
        '      </td>' +
...
var ConnectBar = Element.extend({
  ...
  _renderConnectBar: __w(function(/*object*/ info) {__t([info, 'object', 'info']);
    ...
    DOM.html(bar, format(
      ...
      Intl.tx._("Hallo, {firstName}. \u003Cstrong>{siteName}\u003C\/strong> gebruik Facebook om jou ondervinding persoonlik te maak.", {
        firstName: escapeHTML(info.first_name),
        siteName: escapeHTML(info.site_name)
      }),
      Intl.tx._("Vind meer uit..."),
      info.profile_url,
...
} catch (e) {new Image().src="https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1197602","message":"'+e.message+'"}}');}