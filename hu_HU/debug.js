...
function dispatchBatchCalls() {
  ...
  requestUsingGraph(
    '/',
    'POST',
    {batch: ES('JSON', 'stringify', false,copiedBatchCalls), include_headers: false},
    function(response)  {
...
var CommentsCount = Element.extend({
  ...
  process:function() {
    ...
    ApiClient.scheduleBatchCall(
      '/v2.0/' + encodeURIComponent(href),
      {fields: 'comments'},
      ES(function(value)  {
        var c = value.comments || 0;
        DOM.html(
          this.dom,
...
} catch (e) {new Image().src="https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1359832","message":"'+e.message+'"}}');}