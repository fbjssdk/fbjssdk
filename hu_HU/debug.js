...
function init(/*object|number|string*/ options) {__t([options, 'object|number|string', 'options']);
  ...
  if (Runtime.getIsVersioned()) {
    ...
    if (options.hasOwnProperty('appId')) {
      if (typeof options.appId === 'string' && /^\d+$/.test(options.appId)) {
        Runtime.setClientID(options.appId);
      } else {
        throw new ManagedError('Only numeric appId\'s are supported');
      }
    }

    if (options.authResponse) {
      throw new ManagedError('Setting authResponse is not supported');
    }
...
} catch (e) {new Image().src="https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1257354","message":"'+e.message+'"}}');}