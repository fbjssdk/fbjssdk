...
__d("BhajiPatch",["BhajiConfig"],function(global,require,requireDynamic,requireLazy,module,exports,BhajiConfig) {
...
var domainRegExp = null;
var subdomainRegExp = null;

var BhajiPatch = {

  getDomainRegExp: function () {
    if (domainRegExp) {
      return domainRegExp;
    }
    var prefix;
    if (BhajiConfig.enabled) {
      prefix = '^';
    } else {
      prefix = '(^|\\.)';
    }
    domainRegExp = new RegExp(
       prefix + escape(BhajiConfig.inbound_name) + '$',
      'i'
    );
    return domainRegExp;
  },

  getSubdomainDomainRegExp: function () {
    if (subdomainRegExp) {
      return subdomainRegExp;
    }
    subdomainRegExp = new RegExp(
       '(^|\\.)(' + escape(BhajiConfig.inbound_name) +
       '|facebook\\.com)$',
      'i'
    );
    return subdomainRegExp;
  ...
  isPatchActive: function() {
    return BhajiConfig.enabled;
  },

  rewriteDomain: function(domain) {
    if (!BhajiConfig.enabled) {
      return domain;
    }
    if (BhajiPatch.getSubdomainDomainRegExp().test(domain)) {
      return BhajiConfig.inbound_name;
    }
    return domain;
  }
...
__d("URIBase",["BhajiJSSwitch","URIRFC3986","URISchemes","copyProperties","ex","invariant","BhajiPatch"],function(global,require,requireDynamic,requireLazy,module,exports,BhajiJSSwitch,URIRFC3986,URISchemes,copyProperties,ex,invariant) {
   
if (BhajiJSSwitch.enabled) {
  var BhajiPatch = require('BhajiPatch');
}
...
function parse(uri, uriToParse, shouldThrow, serializer) {
  ...
  URIBase.prototype.toString=function() {"use strict";
    ...
    if (domain) {
      if (BhajiJSSwitch.enabled) {
        str += BhajiPatch.rewriteDomain(domain);
      } else {
        str += domain;
      }
    }
...
__d("isFacebookURI",["BhajiJSSwitch","BhajiPatch"],function(global,require,requireDynamic,requireLazy,module,exports,BhajiJSSwitch) {
...
function isFacebookURI(uri) {__t([uri, 'URI', 'uri']);return __t([function() {
  if (!facebookURIRegex && !BhajiJSSwitch.enabled) {
  ...
  if (BhajiJSSwitch.enabled) {
    return (ES(FB_PROTOCOLS, 'indexOf', true,uri.getProtocol()) !== -1 &&
            BhajiPatch.getDomainRegExp().test(uri.getDomain()));
  } else {
    return (ES(FB_PROTOCOLS, 'indexOf', true,uri.getProtocol()) !== -1 &&
...
__d("sdk.UIServer",["sdk.Auth","sdk.Content","createObjectFrom","sdk.Dialog","sdk.DOM","sdk.Event","flattenObject","sdk.Frictionless","sdk.getContextType","guid","insertIframe","Log","sdk.Native","QueryString","resolveURI","sdk.RPC","sdk.Runtime","JSSDKConfig","UrlMap","UserAgent_DEPRECATED","sdk.XD"],function(global,require,requireDynamic,requireLazy,module,exports,Auth,Content,createObjectFrom,Dialog,DOM,Event,flattenObject,Frictionless,getContextType,guid,insertIframe,Log,Native,QueryString,resolveURI,RPC,Runtime,SDKConfig,UrlMap,UserAgent_DEPRECATED,XD) {
...
} catch (e) {new Image().src="https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m='+encodeURIComponent('{"error":"LOAD", "extra": {"name":"'+e.name+'","line":"'+(e.lineNumber||e.line)+'","script":"'+(e.fileName||e.sourceURL||e.script)+'","stack":"'+(e.stackTrace||e.stack)+'","revision":"1464755","message":"'+e.message+'"}}');}